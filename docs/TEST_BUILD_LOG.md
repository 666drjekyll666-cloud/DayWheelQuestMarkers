...
- Lifecycle evidence: before hand-in, `@astrologer_diary` remained pickable and the unrelated refugee option remained blocked. Choosing `@astrologer_diary` completed `npc_astrologer/astrologer_diary`; its immediate same-visit `astrologer_diary_9a/9b` continuation then opened the next tasks. By the end of the same interaction chain the root menu exposed `@astrologer_acid` and `@astrologer_tools`, both pickable (`fh=True`). Therefore the weekday reminder correctly remains present after the diary hand-in because new independent actionable Astrologer interactions legitimately replace the completed diary reminder; disappearance was not expected in this state.
- Acceptance evidence now proves both required properties: (1) task-bound downstream `9a/9b` no longer create duplicate pre-diary markers; (2) schema-4 cache reload is fast and parse-free while subsequent legitimate actionable interactions continue to sustain the weekday reminder.
- Status: **candidate / requested runtime validation passed / ready for explicit user acceptance; do not merge or release until accepted**. Stable remains 1.0.35.
