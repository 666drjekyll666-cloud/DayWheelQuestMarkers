...
- Persistent manifest schema: **5**. Existing schema-4 cache must rebuild once behind loading; later loads should deserialize schema 5 and skip FlowCanvas parsing.
...
- Requested test:
  ...
  4. restart the game and load again: confirm schema-5 manifest loads quickly with `FlowCanvas graph parse skipped`;
  5. at the preserved/available Snake counterfeit-coins + Restoration Tools state, verify the already-proven behavior remains **2 -> 1 -> 0**. If that exact save state is no longer available, the two log checks still prove compiler/bootstrap integrity but do not replace the behavioral regression test.
