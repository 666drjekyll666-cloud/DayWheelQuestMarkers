...
- Requested test: replace/remove 0.1.1, install 0.1.2 beside accepted production 1.0.30, load any normal gameplay save until the probe prints `AUTO3_END`, then provide `LogOutput.log`. No NPC interaction or special day/task state is required for this provenance scan.
- Player result: **successful / research audit closed**. The probe completed `AUTO3_END` after scanning 92 loaded controllers / 90 unique serialized graphs and 3,397 loaded `TextAsset`s. `morgue_quest` was proven to be fired inside Snake's exact `@snake_give_key` function path (authored key price `ques_key_cultist x1`); `snake_stone_ready` was proven to originate from a selectable `Flow_MultiAnswer` branch and `Flow_FireEvent` before its CustomEvent completion sequence; `on_back_to_snake_after_ritual` was proven to be installed through `Flow_AddInteractionEvent`, with prior authored evidence showing `@snake_sword` (price `sword_damask_gem x1`) makes `snake_back` Visible before that later interaction stage. Inquisitor controls independently showed the same interaction-event mechanism. Final census/design conclusion: 70/72 owner-local completions are ordinary selectable routes once exact FireEvent->CustomEvent topology is represented; the two genuine mandatory interaction-event completion stages are `inquisitor_talk` and `snake_back`.
- Status: **research only / provenance audit complete / not production / main unchanged**.
