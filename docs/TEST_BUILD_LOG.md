...
- Remaining acceptance test: fully restart on the same pre-diary save without touching cache files; schema 4 must load directly with FlowCanvas graph parsing skipped and the Astrologer must still show one marker. Then hand in the diary and verify that this reminder contribution disappears naturally unless a newly actionable independent Astrologer interaction legitimately replaces it. Return one final log covering that run.
- Status: **candidate / first-launch fix proven / awaiting cache-reload + lifecycle acceptance / do not merge or release**. Stable remains 1.0.35.
