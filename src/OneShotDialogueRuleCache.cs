...
{
    /// <summary>
    /// Cached classifier for authored weekday-NPC dialogue topics that are genuinely one-shot.
    ///
    /// A topic is admitted only when its own authored branch persistently blacklists that exact
    /// @phrase. Live phrase state plus the game's own SmartRes sufficiency path then determine
    /// whether the topic is currently actionable. Menu/container topics such as Trade, Leave,
    /// Back and non-consuming "about ..." submenu headers never satisfy the self-consumption rule.
    ///
    /// This is a bounded loading-time parse of the six weekday-NPC graphs. Gameplay evaluation is
    /// only cached topic iteration plus phrase/gate checks; no graph traversal occurs during play.
    /// </summary>
    internal sealed class OneShotDialogueRuleCache
    {
        ...
        internal sealed class TopicRule
        {
            ...
            internal readonly List<GateVariant> Variants = new List<GateVariant>();
        }
        ...
        // 1.0.22 already represents these exact topics through narrow verified bridge/intermediate
        // rules. Keep those accepted implementations during the 1.0.23 behavior expansion and
        // suppress their generic duplicates. This list can be removed together with the legacy
        // supplemental classes after the broader one-shot rule is player-accepted.
        private static readonly HashSet<string> LegacySupplementAnswerIds = new HashSet<string>(StringComparer.Ordinal)
        {
            "@astrologer_fix_mill",
            "@astrologer_fix",
            "@snake_instrument",
            "@snake_instrument_ready",
            "@actress_father",
            "@merchant_ceremony",
            "@inquisitor_portal_guard",
            "@actress_marketing",
            "@actress_ jewelry",
            "@snake_help",
            "@snake_nacklase_0",
            "@snake_nacklase_again",
        ...
        internal int TopicCount { get; private set; }
        ...
        internal bool Build(object save, object mainGame)
        {
            ...
            var knownNpcIds = ReadKnownPeriodicNpcIds(save);
            for (var i = 0; i < NpcIds.Length; i++)
            {
                var npcId = NpcIds[i];
                if (!knownNpcIds.Contains(npcId)) continue;
                ...
                var target = new TargetRules { NpcId = npcId, WorldObject = wgo };
                ParseGraph(target, serialized);
                _targets.Add(target);
            }
        ...
        internal bool TryRebind(object mainGame)
        {
            if (mainGame == null || !TryBindPlayer(mainGame)) return false;
        ...
        internal bool IsActionable(TopicRule topic, object unlockedPhrases, object blacklistedPhrases)
        {
        ...
        private void ParseGraph(TargetRules target, string serialized)
        {
            ...
            // Direct task-completion answers are already represented by the owner-local/cross-owner caches.
            // Collect every completion anchor irrespective of task owner so the generic one-shot layer cannot
            // duplicate an existing quest marker.
            ...
            foreach (var node in nodes.Values)
            {
                ...
                for (var i = 0; i < anchors.Count; i++) AddAnchorAnswerIds(completionAnswerIds, anchors[i], serialized, nodes);
            }
            ...
            foreach (var node in nodes.Values)
            {
                ...
                if (completionAnswerIds.Contains(phraseId) || LegacySupplementAnswerIds.Contains(phraseId)) continue;
            ...
            Dictionary<string, List<Connection>> incomingValue)
        {
            ...
            {
                AddVariant(topic, new GateVariant());
                SupportedRuleCount++;
                return;
            }
            ...
            foreach (var c in answerConnections)
            {
                ...
                if (incomingValue.TryGetValue(answerNode.Id, out gates))
                {
                    foreach (var gate in gates)
                    {
                        ...
                        Node source;
                        if (!nodes.TryGetValue(gate.SourceNode, out source) ||
                        ...
                        if (string.Equals(gate.TargetPort, "price", StringComparison.OrdinalIgnoreCase)) variant.Price = req;
                        else variant.Lock = req;
                    }
                }

                AddVariant(topic, variant);
                if (variant.Unsupported) UnsupportedRuleCount++; else SupportedRuleCount++;
            }

            if (!foundAnswerData)
            {
                AddVariant(topic, new GateVariant { Unsupported = true });
                UnsupportedRuleCount++;
            }
        }

        private static void AddVariant(TopicRule topic, GateVariant variant)
        {
            // Multiple identical graph routes can reach the same persisted topic. Preserve true gate variants,
            // but avoid duplicate default variants which would otherwise create needless recurring checks.
            for (var i = 0; i < topic.Variants.Count; i++)
            {
        ...
        {
            var type = ReadNodeContent(serialized, node, "res_type") ?? ReadNodeContent(serialized, node, "Res type");
            var id = ReadNodeContent(serialized, node, "id") ?? ReadNodeContent(serialized, node, "Id");
            float value;
        ...
        private bool IsEnough(Requirement req)
        {
            ...
            try
            {
                var value = _isEnough.Invoke(_player, new[] { req.SmartRes });
        ...
        private HashSet<string> ReadKnownPeriodicNpcIds(object save)
        {
            ...
            foreach (var npc in npcs)
            {
                ...
                for (var i = 0; i < NpcIds.Length; i++)
                {
                    if (!string.Equals(id, NpcIds[i], StringComparison.Ordinal)) continue;
                    result.Add(id);
                    break;
                }
            }
